...
class WebScraperConnector(BaseConnector):
    ...
    def _search_once(cls, endpoint, query):
        try:
            # DDG's HTML search endpoint is reliably returning the real result
            # links when queried with a normal GET request. The reference
            # research implementation uses this same approach because the
            # POST variant can return a page whose result links are not
            # exposed in the expected `uddg=` form.
